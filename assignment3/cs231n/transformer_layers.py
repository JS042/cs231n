...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        k = self.key(key).view(N,-1,self.num_heads, self.head_dim)     # (N,T,H,E/H)
        q = self.query(query).view(N,-1,self.num_heads, self.head_dim)
        v = self.value(value).view(N,-1,self.num_heads, self.head_dim)

        k = k.transpose(1,2) # (N,H,T,E/H)
        q = q.transpose(1,2)
        v = v.transpose(1,2)

        scores = torch.matmul(q,k.transpose(-2,-1) / math.sqrt(self.head_dim)) # (N,H,T,T)
        ...
        concat = scores.transpose(1,2).contiguous().view(N,-1,self.embed_dim)
