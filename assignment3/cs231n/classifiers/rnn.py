...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            ...
            h, cache_rnn = lstm_forward(out,h0,Wx,Wh,b)

        out, cache_tempaffine = temporal_affine_forward(h,W_vocab,b_vocab)

        loss, dout = temporal_softmax_loss(out, captions_out, mask)

        # backward pass
        dout, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dout, cache_tempaffine)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            if self.cell_type =='rnn':
                ...
                if i == 0:
                    c = np.zeros(h0.shape)
                next_h, c, _ = lstm_step_forward(x,h0,c,Wx,Wh,b)
