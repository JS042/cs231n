...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            if i!=k:
                z_i = out[i]
                sim1 += torch.exp(sim(z_k,z_i) / tau)
            if i!=k+N:
                z_i = out[i]
                sim2 += torch.exp(sim(z_k_N,z_i) / tau)            
 
        
        loss1 = -torch.log(torch.exp(sim(z_k,z_k_N) / tau) / sim1)
        loss2 = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / sim2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    loss = torch.inner(out_left,out_right) / (torch.linalg.norm(out_left,dim=1) * torch.linalg.norm(out_right,dim=1))
    pos_pairs = torch.Tensor((loss[0,0], loss[1,1])).unsqueeze(1)
...
def compute_sim_matrix(out):
    ...
    num = torch.inner(out,out)
    
    for i in range(N):
        for j in range(N):
            denom = torch.linalg.norm(out[i]) * torch.linalg.norm(out[j])
            sim_matrix[i,j] = num[i,j] / denom
