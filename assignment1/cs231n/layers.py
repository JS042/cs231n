...
def affine_forward(x, w, b):
    ...
    out = np.dot(np.reshape(x, [x.shape[0],input_shape]),w) + b
...
def svm_loss(x, y):
    ...
    margin = np.maximum(x - np.reshape(correct_class_scores, [num_train,1]) + 1,0)
    ...
    dx = np.zeros_like(x)
    dx[margin>0] = 1
    dx[range(num_train),y] -= np.sum(margin >0, axis=1)
    dx /= num_train
